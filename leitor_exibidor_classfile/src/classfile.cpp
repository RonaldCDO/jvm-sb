...
void ClassFile::ShowAccessFlags() {
    ...
    if (byte == ACC_PUBLIC_C)  std::cout << "\t\t\tACC_PUBLIC\n" << std::endl;

    byte = access_flags & 0x00F0;

    if (byte == ACC_FINAL_C) std::cout << "\t\t\tACC_FINAL\n" << std::endl;
    if (byte == ACC_SUPER_C) std::cout << "\t\t\tACC_SUPER\n" << std::endl;

    byte = access_flags & 0x0F00;

    if (byte == ACC_INTERFACE_C) std::cout << "\t\t\tACC_INTERFACE\n" << std::endl;
    if (byte == ACC_ABSTRACT_C) std::cout << "\t\t\tACC_ABSTRACT\n" << std::endl;

    byte = access_flags & 0xF000;

    if (byte == ACC_ANNOTATION_C) std::cout << "\t\t\tACC_ANNOTATION\n" << std::endl;
    if (byte == ACC_ENUM_C) std::cout << "\t\t\tACC_ENUM\n" << std::endl;
}
